...
```json
{
    ...
    "input": "String - tool-specific content. For tool=mcp, this must be exactly one API call such as BrowserTools.open_search_engine_settings()."
}
```
...
- Use `mcp` only for one concrete software API call written directly in `input`
...
- For `mcp`, `input` must be exactly one call expression such as `BrowserTools.open_search_engine_settings()` or `CodeTools.goto_file(file_path='/home/user/a.py', line=3, character=1)`
...
class HiSA:
    ...
    def _get_decision_action_fingerprint(self, decision: Dict) -> str:
        ...
        if tool == MCP_TOOL:
            return self._hash_text(str(tool_input or "").strip())
    ...
    def _detect_execution_loop(self, decision: Dict) -> Optional[str]:
        ...
        tool = decision.get("tool", "")
        if tool not in {MCP_TOOL, "gui_action", "bash_execution"} or not self.action_logs:
            return None
    ...
    def _get_global_planner_decision(self) -> Optional[Dict]:
        ...
        while attempt < self.max_parse_retries:
            ...
            try:
                ...
                if decision["tool"] not in [MCP_TOOL, "gui_action", "bash_execution", "wait", "termination", "infeasible"]:
                    raise ValueError(f"Invalid tool: {decision['tool']}")
                if "input" not in decision:
                    decision["input"] = ""
                if decision["tool"] in {MCP_TOOL, "gui_action", "bash_execution", "termination", "infeasible"}:
                ...
                elif decision["tool"] == MCP_TOOL:
                    self._parse_mcp_input(decision["input"])
    ...
    def _execute_tool(self, decision: Dict, planner_usage: Dict) -> Tuple[str, Optional[str], str]:
        ...
        tool_input = decision.get("input", "")
        if tool == MCP_TOOL:
            execution_result_text = self._mcp_call(tool_input)
