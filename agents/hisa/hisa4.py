...
NON_GUI_TOOLS = {MCP_TOOL, "bash_execution", "wait", "termination", "infeasible"}
...
# ==================== PROMPTS ====================
GLOBAL_PLANNER_PROMPT = """You are an expert in GUI interaction, bash execution, and software-level MCP tools. Always keep the task instruction in mind.
...
5. Use the latest screenshot as the source of truth for visible GUI state, but do not trust stale screenshots over direct file/data readback after bash or mcp edits
...
8. Treat `mcp`, `gui_action`, and `bash_execution` as peer tools. Choose the one that best fits the current subgoal.
9. If bash or mcp already wrote the target state and a later bash/mcp readback verified the requested result, do not add GUI Save, reopen, reload, or refresh steps just to make the stale window look updated. Prefer termination once the task is verified complete.

# Tools
## mcp
Call one software-level API exposed for the current domain. Use this when the task maps cleanly to a domain-specific software operation.
...
```json
{
    ...
    "actions": [
        {
            "tool": "mcp|gui_action|bash_execution|wait|termination|infeasible",
...
- Prefer one meaningful interaction per action item. Even though some tools can execute multiple low-level operations inside one input, you should usually split sequential interactions into separate `actions` items for clarity and better replanning.
- Use `mcp` only for one concrete software API call per action item written directly in `input`
- Use `gui_action` when the visible GUI is the main source of truth
- Use `bash_execution` for file/system work and precise textual verification
- You may mix tools freely inside `actions`
- Good example:
```json
{
  "thought": "Select Bing and then confirm the default search engine change.",
  "subgoal": "Set Bing as the default search engine",
  "actions": [
    {"tool": "gui_action", "input": "pyautogui.click(695, 514) # click the radio button next to Microsoft Bing"},
    {"tool": "gui_action", "input": "pyautogui.click(663, 807) # click Set as default button"}
  ]
}
```
...
- Always include a non-empty `actions` list
- Prefer splitting sequential GUI interactions into separate `actions` items instead of combining multiple clicks into one `gui_action` input
- Use `mcp` only when a listed domain API clearly matches the step
- For `mcp`, each `actions[i].input` must be exactly one call expression such as `BrowserTools.open_search_engine_settings()` or `CodeTools.goto_file(file_path='/home/user/a.py', line=3, character=1)`
...
- If no further action is needed because the task is already verified complete, add a `termination` action instead of an empty `gui_action`
- Do not wrap multiple MCP calls into one `input`; use multiple `actions` items instead"""
...
- The screenshot is primary evidence for the visible final state. If the screenshot does not directly show the requested result, return `fail`.
- Use the logs to confirm what was actually modified, saved, read back, or verified. If logs only show that a command or API call ran, but do not confirm the requested final result, return `fail`.
- If a file was modified from bash while the desktop app may still display stale content, prefer explicit readback/verification evidence from the logs plus the latest screenshot of the reopened app state.
...
- If bash or mcp already verified the requested file/data state, do not suggest GUI Save, reopen, reload, or refresh just to sync a stale window; suggest termination or another text-level verification instead.
...
class HiSA:
    ...
    def __init__(
        ...
        self.mcp_registry = MCPRegistry(os.path.dirname(__file__))
    ...
    def _build_planner_system_prompt(self) -> str:
        ...
        for shared_skill_name in ["mcp.md", "gui.md", "bash.md"]:
            if self._skill_file_exists(shared_skill_name):
                sections.append(self._load_skill_text(shared_skill_name))
        mcp_domains = [getattr(self, "current_domain", "")]
        mcp_domains.extend(getattr(self, "task_related_domains", []) or [])
        mcp_prompt = self.mcp_registry.render_prompt(mcp_domains)
        if mcp_prompt:
    ...
    def _should_attach_planner_screenshot(self) -> bool:
        for log in reversed(self.action_logs):
            tool_type = str(log.get("type", "")).strip().lower()
            if not tool_type or tool_type == "mode_block":
                continue
            return tool_type not in {"bash_execution", "mcp"}
        return True
    ...
    def _derive_execution_status(self, decision: Dict) -> Tuple[str, str]:
        ...
        if any(action.get("tool") == "termination" for action in decision.get("actions", [])):
            return "finish", ""
    ...
    def _record_subgoal_transition(self, decision: Dict) -> str:
        ...
        for log in reversed(self.action_logs):
            ...
            if blocking_reason:
                log["blocking_reason"] = blocking_reason
            else:
                log.pop("blocking_reason", None)
    ...
    def _get_last_meaningful_tool(self) -> str:
        for log in reversed(self.action_logs):
            ...
            if tool_type in {"bash_execution", "gui_action", MCP_TOOL}:
                return tool_type
        return ""
    ...
    def _maybe_reopen_office_file_before_final_verification(self, task_config: dict) -> None:
        domain = str(task_config.get("domain") or getattr(self, "current_domain", "") or "").strip().lower()
        if domain not in {"libreoffice_calc", "libreoffice_writer", "libreoffice_impress"}:
            return
        ...
        self.logger.info(
            "Pre-final-verification office reload: closing %s and reopening %s after bash-based file edits.",
            window_name,
            file_path,
        )
        try:
            self.env.setup_controller.setup(
                ...
                getattr(self.env, "enable_proxy", False),
            )
            self.env.controller.execute_python_command("pyautogui.press('enter')")
            time.sleep(0.5)
            self.env.setup_controller.setup(
                [
                    {
    ...
    def _run_final_verification(self) -> bool:
        screenshot = self._wait_until_screenshot_available(timeout_seconds=1.5)
        ...
        if screenshot_b64:
            ...
            self.logger.warning("Final verification is proceeding without a current screenshot.")
    ...
    def _get_decision_action_fingerprint(self, decision: Dict) -> str:
        ...
        for action in decision.get("actions", []):
            ...
            normalized_actions.append({"tool": tool, "input": tool_input})
        if not normalized_actions:
            return ""
        return self._hash_text(json.dumps(normalized_actions, sort_keys=True, ensure_ascii=False))
    ...
    def _detect_repeated_gui_description(self, decision: Dict) -> Optional[str]:
        ...
        if len(actions) != 1 or actions[0].get("tool") != "gui_action":
            return None

        normalized_description = self._hash_text(str(actions[0].get("input", "") or "").strip())
    ...
    def _detect_execution_loop(self, decision: Dict) -> Optional[str]:
        ...
        actionable_tools = [tool for tool in tools if tool in {MCP_TOOL, "gui_action", "bash_execution"}]
        ...
        while attempt < self.max_parse_retries:
            ...
            try:
                ...
                for index, action in enumerate(actions):
                    ...
                    if tool not in [MCP_TOOL, "gui_action", "bash_execution", "wait", "termination", "infeasible"]:
                        raise ValueError(f"Invalid tool in action #{index + 1}: {tool}")
                    action_input = action.get("input", "")
                    if tool in {MCP_TOOL, "gui_action", "bash_execution", "termination", "infeasible"}:
                        if not isinstance(action_input, str) or not str(action_input).strip():
                            if tool == "gui_action":
                                raise ValueError(
                                    f"Action #{index + 1} input must be a non-empty string for tool=gui_action. "
                                    "If the task is already verified complete, use a termination action instead of an empty gui_action."
                                )
                            raise ValueError(f"Action #{index + 1} input must be a non-empty string for tool={tool}")
                    elif tool == "wait":
                        if not isinstance(action_input, (str, int, float)):
                            raise ValueError(f"Action #{index + 1} input must be string or number for tool=wait")
                    if tool == "gui_action":
                        self._parse_pyautogui_code(str(action_input))
                    elif tool == MCP_TOOL:
                        self._parse_mcp_input(str(action_input))
                    normalized_actions.append({"tool": tool, "input": action_input})

                decision["actions"] = normalized_actions
                decision["subgoal"] = self._resolve_planner_subgoal(decision["subgoal"], normalized_actions[-1]["tool"])
    ...
    def _execute_tool(self, decision: Dict, planner_usage: Dict) -> Tuple[str, Optional[str], str]:
        ...
        gui_inputs = [
            str(action.get("input", "") or "").strip()
            for action in decision.get("actions", [])
            if action.get("tool") == "gui_action"
        ]
        ...
        for action in decision.get("actions", []):
            ...
            tool_input = action.get("input", "")
            if tool == MCP_TOOL:
                action_output = self._mcp_call(tool_input)
                ...
                action_output = self._wait(tool_input)
            elif tool == "termination":
                terminal_status = "termination"
                terminal_message = str(tool_input or "")
                break
    ...
    def _parse_mcp_input(self, code: str) -> Dict[str, Any]:
        text = str(code or "").strip()
        if not text:
            raise ValueError("Decision 'input' must be a non-empty string for tool=mcp")
        try:
            tree = ast.parse(text, mode="eval")
        except SyntaxError as e:
            raise ValueError(f"Failed to parse mcp call: {e}") from e
        expr = tree.body
        if not isinstance(expr, ast.Call):
            raise ValueError("mcp input must be exactly one call expression")
        if not isinstance(expr.func, ast.Attribute) or not isinstance(expr.func.value, ast.Name):
            raise ValueError("mcp input must look like ClassName.method_name(...)")
        ...
        for index, arg in enumerate(expr.args):
            try:
                ...
                raise ValueError(
                    f"mcp positional argument #{index + 1} must be a Python literal value"
                ) from e
        arguments: Dict[str, Any] = {}
        for kw in expr.keywords:
            if kw.arg is None:
                raise ValueError("mcp input does not support **kwargs")
            try:
                arguments[kw.arg] = ast.literal_eval(kw.value)
            except Exception as e:
                raise ValueError(
                    f"mcp argument `{kw.arg}` must be a Python literal value"
                ) from e
        return {
    ...
    def _mcp_call(self, code: str) -> str:
        """Execute an MCP call via setup controller, direct GUI shortcut, or VM-side handler."""
        step_start_time = time.time()
        step = self.current_step_id or (self.operation_count + 1)
        screenshot_file = f"step_{step}.png"
        parsed_call = self._parse_mcp_input(code)
        call_desc = str(code or "").strip()
        requested_action_fingerprint = self._hash_text(call_desc)
        self.logger.info("[mcp] %s", call_desc)

        before_screenshot = self.env.controller.get_screenshot()
        if before_screenshot is None:
            raise RuntimeError("Failed to capture screenshot before mcp execution")

        try:
            execution = self.mcp_registry.invoke(
                getattr(self, "current_domain", ""),
                parsed_call["name"],
            ...
            if execution_mode == "setup":
                ...
                success = self.env.setup_controller.setup(setup_payload, False)
                if success is False:
                ...
                if not gui_code:
                    raise ValueError(f"mcp gui payload is empty for {call_desc}")
                ...
                raise ValueError(f"Unsupported mcp execution mode: {execution_mode}")
            ...
            if execution_mode == "vm" and vm_status != "success":
                blocked_hint = vm_logs or f"status={vm_status}"
            elif not env_changed:
                blocked_hint = f"no_visible_change after {wait_elapsed:.1f}s"

            if execution_mode == "vm":
                step_abstraction_summary = self._step_abstraction(
                    before_screenshot=None,
                    after_screenshot=None,
                    action_description=f"mcp call: {call_desc}",
            ...
            self.action_logs.append({
                ...
                "compact": self._build_compact_log_entry(
                    ...
                    next_hint="Use another peer tool only if this domain API path does not cover the next missing step."
                ),
            ...
            self.logger.error("mcp execution error: %s", e)
            step_time = time.time() - step_start_time
            result_fingerprint = self._hash_text(f"mcp_error={str(e)}")
            try:
                screenshot = self.env.controller.get_screenshot()
                if screenshot is not None:
                    with open(os.path.join(self.operations_dir, screenshot_file), "wb") as f:
            ...
            self.action_logs.append({
                ...
                "compact": self._build_compact_log_entry(
                    ...
                    next_hint="Either fix the MCP call arguments or switch to bash/gui for the same subgoal."
                ),
                "step_time": round(step_time, 2),
                "token_usage": self.step_token_usage,
                "loop_action_fingerprint": requested_action_fingerprint,
                ...
                "decision_gui_fingerprint": self.current_decision_gui_fingerprint,
                "decision_result_fingerprint": self._hash_text(
                    f"status=blocked|mcp_error={str(e)}|detail={call_desc}"
                ),
            })
            self.last_blocked_feedback_event = {
                "type": "tool_execution_failed",
...
"""
    ...
    def _evaluate_and_save(self, task_config: dict, additional_context: str,
        ...
        mcp_steps = len({
            int(log.get("step"))
            for log in self.action_logs
            if log.get("type") == MCP_TOOL and log.get("step") is not None
        })
        bash_steps = len({
            int(log.get("step"))
            for log in self.action_logs
