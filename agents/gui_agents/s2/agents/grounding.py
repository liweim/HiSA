...
from agents.gui_agents.s2.core.mllm import LMMAgent
...
class OSWorldACI(ACI):
    def __init__(
        ...
        # Store observation (screenshot and other info) for CUA usage
        self.obs = None
    ...
    def _get_code_from_cua(self, element_description: str) -> str:
        """
        Generates PyAutoGUI code using the CUA model based on the element description.
        """
        cua_agent = self.grounding_model
        cua_agent.reset()
        cua_agent.add_system_prompt(CUA_SYSTEM_PROMPT)
        cua_agent.add_message(
            text_content=element_description, image_content=self.obs["screenshot"]
        )
        return call_llm_safe(cua_agent)
    ...
    def assign_coordinates(self, plan: str, obs: Dict):
        ...
        # Store obs for CUA model usage
        self.obs = obs

        # Reset coords from previous action generation
        self.coords1, self.coords2 = None, None

        # Check if using CUA for grounding - if so, skip coordinate generation
        # CUA model will handle everything in the @agent_action functions
        is_cua_grounding = self.engine_params_for_grounding.get("model") == "computer-use-preview"
        if is_cua_grounding:
            return  # Skip coordinate assignment for CUA
    ...
    def click(
        ...
        """
        # Check which model is being used for grounding
        if self.engine_params_for_grounding.get("model") == "computer-use-preview":
            # If it's CUA, use the new common function
            return self._get_code_from_cua(f"Click on {element_description}")
        else:
    ...
    def type(
        ...
        if self.engine_params_for_grounding.get("model") == "computer-use-preview":
            # If it's CUA, use the new common function
            description = f"Type '{text}' into {element_description if element_description else 'the element'}"
            return self._get_code_from_cua(description)
        else:
            # Otherwise, use the original logic
    ...
    def drag_and_drop(
        ...
        if self.engine_params_for_grounding.get("model") == "computer-use-preview":
            # If it's CUA, use the new common function
            description = f"Drag from {starting_description} to {ending_description}"
            return self._get_code_from_cua(description)
        else:
            # Otherwise, use the original logic
    ...
    def highlight_text_span(self, starting_phrase: str, ending_phrase: str, button: str = "left"):
        ...
        if self.engine_params_for_grounding.get("model") == "computer-use-preview":
            # If it's CUA, use the new common function
            description = f"Highlight text from '{starting_phrase}' to '{ending_phrase}'"
            return self._get_code_from_cua(description)
        else:
            # Otherwise, use the original logic
            x1, y1 = self.coords1
    ...
    def scroll(self, element_description: str, clicks: int, shift: bool = False):
        ...
        if self.engine_params_for_grounding.get("model") == "computer-use-preview":
            ...
            description = f"Scroll {direction} on {element_description}"
            return self._get_code_from_cua(description)
        else:
            x, y = self.resize_coordinates(self.coords1)
