...
class Manager(BaseModule):
    ...
    def get_usage_stats(self):
        """Get detailed usage statistics from all LMM agents in Manager"""
        stats_dict = {
            'generator_agent': UsageStats(),
            'dag_translator_agent': UsageStats(),
            'narrative_summarization_agent': UsageStats(),
            'episode_summarization_agent': UsageStats(),
        }
        
        agents_map = {
            'generator_agent': self.generator_agent,
            'dag_translator_agent': self.dag_translator_agent,
            'narrative_summarization_agent': self.narrative_summarization_agent,
            'episode_summarization_agent': self.episode_summarization_agent,
        }
        
        for agent_name, agent in agents_map.items():
            if hasattr(agent, 'engine') and hasattr(agent.engine, 'get_usage_stats'):
                stats_dict[agent_name] = agent.engine.get_usage_stats()
