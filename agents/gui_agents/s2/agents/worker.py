...
class Worker(BaseModule):
    ...
    def reset(self):
        # Save existing usage stats before resetting
        saved_generator_stats = None
        saved_reflection_stats = None

        if hasattr(self, 'generator_agent') and hasattr(self.generator_agent, 'engine'):
            if hasattr(self.generator_agent.engine, 'get_usage_stats'):
                saved_generator_stats = self.generator_agent.engine.get_usage_stats()

        if hasattr(self, 'reflection_agent') and hasattr(self.reflection_agent, 'engine'):
            if hasattr(self.reflection_agent.engine, 'get_usage_stats'):
                saved_reflection_stats = self.reflection_agent.engine.get_usage_stats()
        ...
        # Restore saved usage stats after creating new agents
        if saved_generator_stats and hasattr(self.generator_agent, 'engine'):
            if hasattr(self.generator_agent.engine, 'usage_stats'):
                # Copy the stats to preserve them across reset
                self.generator_agent.engine.usage_stats.prompt_tokens = saved_generator_stats.prompt_tokens
                self.generator_agent.engine.usage_stats.completion_tokens = saved_generator_stats.completion_tokens
                self.generator_agent.engine.usage_stats.image_count = saved_generator_stats.image_count

        if saved_reflection_stats and hasattr(self.reflection_agent, 'engine'):
            if hasattr(self.reflection_agent.engine, 'usage_stats'):
                # Copy the stats to preserve them across reset
                self.reflection_agent.engine.usage_stats.prompt_tokens = saved_reflection_stats.prompt_tokens
                self.reflection_agent.engine.usage_stats.completion_tokens = saved_reflection_stats.completion_tokens
                self.reflection_agent.engine.usage_stats.image_count = saved_reflection_stats.image_count
    ...
    def get_usage_stats(self):
        """Get detailed usage statistics from all LMM agents in Worker"""
        stats_dict = {
            'generator_agent': UsageStats(),
            'reflection_agent': UsageStats(),
        }
        
        # Get generator agent stats
        if hasattr(self.generator_agent, 'engine') and hasattr(self.generator_agent.engine, 'get_usage_stats'):
            stats_dict['generator_agent'] = self.generator_agent.engine.get_usage_stats()
        
        # Get reflection agent stats
        if hasattr(self.reflection_agent, 'engine') and hasattr(self.reflection_agent.engine, 'get_usage_stats'):
            stats_dict['reflection_agent'] = self.reflection_agent.engine.get_usage_stats()
