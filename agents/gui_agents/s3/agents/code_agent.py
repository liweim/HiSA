...
from agents.gui_agents.s3.utils.common_utils import call_llm_safe, split_thinking_response
...
class CodeAgent:
    ...
    def execute(self, task_instruction: str, screenshot: str, env_controller, 
                current_step: int = 0, max_steps: int = float('inf')) -> Dict:
        """Execute code for the given task with a budget of steps.
        
        Args:
            task_instruction: The task to execute
            screenshot: Screenshot for context
            env_controller: Controller for executing code
            current_step: Current step count from the outer loop
            max_steps: Maximum total steps allowed (outer loop limit)
        """
        ...
        print(f"Code agent budget: {self.budget} steps")
        print(f"Current outer step: {current_step}, Max outer steps: {max_steps}")
        print(f"Remaining steps: {max_steps - current_step}")
        print("=" * 60)

        logger.info(f"Starting code execution for task: {task_instruction}")
        logger.info(f"Code agent budget: {self.budget} steps")
        logger.info(f"Current outer step: {current_step}, Max outer steps: {max_steps}")
        ...
        while step_count < self.budget:
            # Check if continuing would exceed max_steps
            total_steps_if_continue = current_step + 1 + step_count  # +1 for the GUI action that called code agent
            if total_steps_if_continue >= max_steps:
                print(f"STOPPING CODE AGENT EARLY")
                print("=" * 60)
                print(f"Total steps would exceed max_steps: {total_steps_if_continue} >= {max_steps}")
                print(f"Code agent executed {step_count} steps before stopping")
                print("=" * 60)
                logger.warning(f"Code agent stopping early: total_steps {total_steps_if_continue} would exceed max_steps {max_steps}")
                completion_reason = f"MAX_STEPS_EXCEEDED_AFTER_{step_count}_CODE_STEPS"
                break
            
            logger.info(f"Code agent step {step_count + 1}/{self.budget} (Total: {current_step + 1 + step_count}/{max_steps})")
