...
GTA1_API_KEY = os.environ.get("GTA1_API_KEY","empty") #"Your GTA1 API Key"
GTA1_MODEL_NMAE = os.environ.get("GTA1_API_KEY","gta1-7b")  #Your served model name
GTA1_SERVICE_URL = os.environ.get("GTA1_SERVICE_URL","http://localhost:1234/v1") #"Your GTA1 Service URL"
...
class GTA1Agent:
    ...
    def __init__(
        ...
        # Create grounding agent instance with specified dimensions
        self.agent = OSWorldACI(platform='linux', width=screen_width, height=screen_height)
    ...
    def predict(self, instruction: str, obs: Dict) -> List:
        ...
        for bn in split_to_batches(N, batch_size=8):
            planner_response_, usage = self.call_llm({
            ...
            # Track planner usage
            if usage:
                if "planner" not in self.model_usage:
                    self.model_usage["planner"] = {"cost": 0.0, "prompt_tokens": 0, "completion_tokens": 0, "image_count": 0}
                prompt_tokens = usage.get("prompt_tokens", 0)
                completion_tokens = usage.get("completion_tokens", 0)
                prompt_price, completion_price = get_price(self.planner_model)
                # Count images in messages
                image_count = sum(1 for msg in messages if isinstance(msg.get("content"), list) and any(item.get("type") == "image_url" for item in msg["content"]))
                cost = prompt_tokens * prompt_price + completion_tokens * completion_price
                self.model_usage["planner"]["cost"] += cost
                self.model_usage["planner"]["prompt_tokens"] += prompt_tokens
                self.model_usage["planner"]["completion_tokens"] += completion_tokens
                self.model_usage["planner"]["image_count"] += image_count
        ...
        logger.info(f"Parsed codes from planner: {codes}")
        ...
        logger.info(f"Parsed observation: {observation_caption}")
        ...
        def request_vllm(image, prompt):
            ...
            if hasattr(response, 'usage') and response.usage:
                if "grounding" not in self.model_usage:
                    self.model_usage["grounding"] = {"cost": 0.0, "prompt_tokens": 0, "completion_tokens": 0, "image_count": 0}
                prompt_tokens = response.usage.prompt_tokens if hasattr(response.usage, 'prompt_tokens') else 0
                completion_tokens = response.usage.completion_tokens if hasattr(response.usage, 'completion_tokens') else 0
                try:
                    prompt_price, completion_price = get_price(GTA1_MODEL_NMAE)
                    cost = prompt_tokens * prompt_price + completion_tokens * completion_price
                    self.model_usage["grounding"]["cost"] += cost
                except:
                    # If get_price fails for this model, just track tokens
                    pass
                self.model_usage["grounding"]["prompt_tokens"] += prompt_tokens
                self.model_usage["grounding"]["completion_tokens"] += completion_tokens
                self.model_usage["grounding"]["image_count"] += 1
        ...
        exec_globals = {"agent": self.agent, "__builtins__": __builtins__}
        pyautogui_actions = [eval(plan_code, exec_globals)]
        ...
        # Return response as a dictionary to match run_single.py expectations
        response = {
            "thought": thought,
            "observation": observation_caption,
            "plan_code": plan_code,
    ...
    def select(self, instruction, screenshot, response, history_cache):
        ...
        for _ in range(MAX_RETRY_TIMES):
            try:
                prediction_response = requests.post(url, headers=headers, json=payload, proxies=proxies, timeout=180)
                if prediction_response.status_code != 200:
                    continue
                prediction_json = prediction_response.json()
                prediction = prediction_json['choices'][0]['message']['content']

                # Track judge usage
                usage = prediction_json.get('usage', {})
                if usage:
                    if "judge" not in self.model_usage:
                        self.model_usage["judge"] = {"cost": 0.0, "prompt_tokens": 0, "completion_tokens": 0, "image_count": 0}
                    prompt_tokens = usage.get("prompt_tokens", 0)
                    completion_tokens = usage.get("completion_tokens", 0)
                    prompt_price, completion_price = get_price(payload["model"])
                    # Count images in messages
                    image_count = sum(1 for msg in messages if any(item.get("type") == "image_url" for item in msg.get("content", [])))
                    cost = prompt_tokens * prompt_price + completion_tokens * completion_price
                    self.model_usage["judge"]["cost"] += cost
                    self.model_usage["judge"]["prompt_tokens"] += prompt_tokens
                    self.model_usage["judge"]["completion_tokens"] += completion_tokens
                    self.model_usage["judge"]["image_count"] += image_count
    ...
    def call_llm(self, payload, model):
        if model.startswith("gpt") or "o3" in model:
            ...
            if response.status_code != 200:
                ...
                return [response["choices"][i]["message"]["content"] for i in range(len(response["choices"]))], usage
        else:
           raise SystemExit
